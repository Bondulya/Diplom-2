...
public class User {
    ...
    public ValidatableResponse apiUserRegister(boolean dropField){
        ...
        ValidatableResponse response = given().header(ApiShop.API_POST_HEADER_TYPE, ApiShop.API_POST_HEADER_VALUE).and()
                .body(json).when().post(ApiShop.API_USER_REGISTER).then();
    ...
    public ValidatableResponse apiLogin(String strLogin, String strPassword){
        ...
        ValidatableResponse response = given()
                .header(ApiShop.API_POST_HEADER_TYPE,ApiShop.API_POST_HEADER_VALUE)
                .and().body(json)
                .when().post(ApiShop.API_LOGIN).then();
        if(response.extract().statusCode() == SC_OK){
            setAccessToken(response);
            setRefreshToken(response);
        }
    ...
    public ValidatableResponse apiDelete(){
        ...
        if(isLogin){
            response = given().auth().oauth2(accessToken).and().when().delete(ApiShop.API_DELETE).then();
        }
        return response;
    }
